...
import { 
  LayoutGrid, Calendar, Users, Settings, LogOut, Menu, Stethoscope, Bell, X 
} from 'lucide-react';
...
export default function SecretaryLayout() {
  ...
  const getSecretaryInfo = () => {
    ...
    if (user) {
      try {
        ...
        return {
          name: parsed.name || parsed.fullName || 'پذیرش مطب',
          initial: (parsed.name || parsed.fullName || 'S').charAt(0).toUpperCase()
        };
      } catch {
        return { name: 'پذیرش مطب', initial: 'S' };
      }
    }
    return { name: 'پذیرش مطب', initial: 'S' };
  };
  ...
  // منوی اختصاصی و ثابت برای منشی مطب
  const menuItems = [
    { title: 'میز کار زنده', icon: LayoutGrid, path: '/secretary/dashboard' },
    { title: 'تقویم نوبت‌دهی', icon: Calendar, path: '/secretary/appointments' },
    { title: 'لیست بیماران', icon: Users, path: '/secretary/patients' },
    { title: 'تنظیمات مطب', icon: Settings, path: '/secretary/settings' },
  ];
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 py-5 border-b border-surface-border">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-0.5">میز کار پذیرش مطب</p>
          </div>
        ...
        <div className="mx-3 mt-3 px-4 py-3 bg-surface-muted rounded-xl">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-primary-100 text-primary-700 font-bold flex items-center justify-center text-sm">
              {secretaryInfo.initial}
            </div>
            <div className="min-w-0">
              <p className="text-sm font-semibold text-slate-800 truncate">{secretaryInfo.name}</p>
              <p className="text-[11px] text-slate-500 font-medium">پذیرش مطب / منشی</p>
            </div>
          </div>
        </div>
